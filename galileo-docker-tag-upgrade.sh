...
while IFS= read -r IMAGE_NAME; do
    if [ -n "$IMAGE_NAME:1.1.5" ]; then
        # Eliminar el prefijo "galileo/" del nombre de la imagen
        CLEANED_NAME="${IMAGE_NAME#galileo/}"
        OUTPUT_FILE="${DEST_DIR}/${CLEANED_NAME}_1.1.6.tar.gz"
        docker tag "${IMAGE_NAME}":1.1.5 "${IMAGE_NAME}":1.1.6
        echo "Guardando la imagen '$IMAGE_NAME':1.1.6 en '$OUTPUT_FILE'..."
        docker save -o "$OUTPUT_FILE" "${IMAGE_NAME}:1.1.6"
    else
